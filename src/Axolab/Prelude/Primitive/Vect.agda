...
data Vect {ℓ} (A : Setoid ℓ) : Nat → Setoid ℓ where
  nil : Vect A 0
  _∷_ : {n : Nat} → A → Vect A n → Vect A (1 + n)

_‼_ : ∀ {ℓ} {A : Setoid ℓ} {n : Nat} → Vect A n → Fin n → A
x ∷ xs ‼ fzero  = x
x ∷ xs ‼ fsuc i = xs ‼ i

map : ∀ {ℓ ℓ'}  {A : Setoid ℓ} {B : Setoid ℓ'} {n : Nat} → (A → B) → Vect A n → Vect B n
map f nil      = nil
