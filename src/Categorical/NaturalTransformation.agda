...
-- Invoke UIP to assert that natural transformations are equivalence when their component-wise assignments are
-- equivalent.
equivNT : {C : Category o ℓ} {D : Category o' ℓ'} {F G : Functor C D} {α β : F ⇒ G} → η α ≡ η β → α ≡ β
equivNT refl = ap (λ e → record { η = _; natural = e }) inside where
      inside = funExt λ _ → funExt λ _ → funExt λ _ → UIP _ _

id-η← : {F G : Functor C D} {α : F ⇒ G} → idNT ∘⇑ α ≡ α
id-η← {D = D} = equivNT (funExt λ _ → D.id←) where
  module D = Category D
...
module _ {F G H I : Functor C D} {α : F ⇒ G} {β : G ⇒ H} {γ : H ⇒ I} where
  ...
  assoc-η→ : (γ ∘⇑ β) ∘⇑ α ≡ γ ∘⇑ (β ∘⇑ α)
  assoc-η→ = equivNT (funExt λ _ → D.assoc→)
